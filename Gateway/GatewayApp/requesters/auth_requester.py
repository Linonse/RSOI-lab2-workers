...
class AuthRequester(Requester):
    ...
    def _delete_users_stories(self, request) -> Tuple[dict, int]:
        from GatewayApp.requesters.stories_requester import StoriesRequester
        mrequester = StoriesRequester()
        user_json, code = self.get_user_info(request)
        if code != 200:
            return user_json, code
        stories_json, code = mrequester.get_stories(request)
        if code != 200:
            return stories_json, code
        for storie in stories_json:
            mrequester.delete_storie(request, storie['uuid'])
        return {}, 204

    def delete_user(self, request) -> Tuple[dict, int]:
        djson, code = self._delete_users_stories(request)
        if code != 204:
            return djson, code
        token = self.get_token_from_request(request)
