...
class StickersRequester(Requester):
    HOST = Requester.BASE_HOST + ':8003/api/stickers/'
    ...
    def get_stickers(self, request) -> Tuple[dict, int]:
        host = self.HOST
        ...
        try:
            ...
            return self.PB_ERROR_RETURN('Stickers')
        response_json = self.next_and_prev_links_to_params(self.get_valid_json_from_response(response))
        Queue.fire_sticker_tasks()
        return response_json, response.status_code

    def get_concrete_sticker(self, request, uuid: str) -> Tuple[dict, int]:
        try:
    ...
    def post_sticker(self, request, data: dict) -> Tuple[dict, int]:
        try:
            response = self._perform_post_request(self.HOST, data=data)
        except ValueError:
            Queue.add_sticker_task(request, data=data)
            return self.ERROR_RETURN[0], 201
        except pb.CircuitBreakerError:
            Queue.add_sticker_task(request, data=data)
            return self.PB_ERROR_RETURN('Stickers')[0], 201
        Queue.fire_sticker_tasks()
        return self.get_valid_json_from_response(response), response.status_code

    def patch_sticker(self, request, uuid: str, data: dict) -> Tuple[dict, int]:
        try:
            response = self._perform_patch_request(self.HOST + f'{uuid}/', data=data)
        except ValueError:
            Queue.add_sticker_task(request, uuid=uuid, data=data)
            return self.ERROR_RETURN[0], 202
        except pb.CircuitBreakerError:
            Queue.add_sticker_task(request, uuid=uuid, data=data)
            return self.PB_ERROR_RETURN('Stickers')[0], 202
        Queue.fire_sticker_tasks()
        return self.get_valid_json_from_response(response), response.status_code

    def delete_sticker(self, request, uuid: str) -> Tuple[dict, int]:
        try:
            response = self._perform_delete_request(self.HOST + f'{uuid}/')
        except ValueError:
            Queue.add_sticker_task(request, uuid=uuid)
            return self.ERROR_RETURN[0], 204
        except pb.CircuitBreakerError:
            Queue.add_sticker_task(request, uuid=uuid)
            return self.PB_ERROR_RETURN('Stickers')[0], 204
        Queue.fire_sticker_tasks()
        return self.get_valid_json_from_response(response), response.status_code
